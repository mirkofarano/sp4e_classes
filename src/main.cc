...
int main(int argc, char *argv[]) {
  ...
  if (argc != 6) {
    std::cerr << "Usage : main P(pi)|A(arithmetic) number separator "
                 "S(screen)|F(file) precision"
              << std::endl;
    return 1;
  }
  ...
  if (separator.compare(",") && separator.compare("|") &&
      separator.compare("")) {
    std::cerr << "Separator must be comma ',' space ' ' or pipe '|' "
              << std::endl;
    return 1;
  }
  ...
  if (PA.compare("P") == 0) {
    ...
    std::cerr
        << "First argument should be P(compute pi) or A(compute arithmetic) "
        << std::endl;
    return 1;
  }
  ...
  if (SF.compare("S") == 0) {
    ...
    std::cerr
        << "Last argument should be S (Prrint on screen) or F (Write on flie) "
        << std::endl;
    return 1;
  }
